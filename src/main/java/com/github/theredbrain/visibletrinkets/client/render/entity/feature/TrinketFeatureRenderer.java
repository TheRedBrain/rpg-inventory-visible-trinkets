package com.github.theredbrain.visibletrinkets.client.render.entity.feature;
...
public class TrinketFeatureRenderer<T extends LivingEntity, M extends BipedEntityModel<T>, A extends BipedEntityModel<T>>
    ...
    private void renderTrinkets(MatrixStack matrices, VertexConsumerProvider vertexConsumers, T entity, String slotGroup, String slotName, A model, boolean slim, int light) {
        ...
        if (!(item instanceof VisibleTrinketItem)) {
            return;
        }
    ...
    private void renderTrinket(MatrixStack matrices, VertexConsumerProvider vertexConsumers, T entity, String slotGroup, String slotName, int light, ItemStack itemStack, A model, boolean slim, float red, float green, float blue) {
        VertexConsumer vertexConsumer = vertexConsumers.getBuffer(RenderLayer.getArmorCutoutNoCull(this.getTrinketTexture((VisibleTrinketItem)itemStack.getItem(), slim)));
        VisibleTrinketsRenderProvider.of(itemStack).getGenericTrinketModel(entity, itemStack, slotGroup, slotName, (BipedEntityModel<LivingEntity>)model, slim).render(matrices, vertexConsumer, light, OverlayTexture.DEFAULT_UV, red, green, blue, 1.0f);
    }

    private Identifier getTrinketTexture(VisibleTrinketItem visibleTrinketItem, boolean slim) {
        String string = visibleTrinketItem.assetSubPath.getNamespace() + ":textures/item/" + visibleTrinketItem.assetSubPath.getPath() + (slim ? "_slim" : "") + ".png";
