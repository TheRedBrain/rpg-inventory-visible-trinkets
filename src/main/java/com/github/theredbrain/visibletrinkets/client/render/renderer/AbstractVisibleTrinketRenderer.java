...
import com.github.theredbrain.visibletrinkets.azurelib.VisibleTrinketsDataTickets;
import com.github.theredbrain.visibletrinkets.azurelib.Trinket;
...
public abstract class AbstractVisibleTrinketRenderer<T extends Item & GeoItem> extends BipedEntityModel implements GeoRenderer<T> {
    ...
    public AbstractVisibleTrinketRenderer(GeoModel<T> model) {
     ...
     */
    public AbstractVisibleTrinketRenderer<T> addRenderLayer(GeoRenderLayer<T> renderLayer) {
     ...
     */
    public AbstractVisibleTrinketRenderer<T> withScale(float scale) {
     ...
     * Sets a scale override for this renderer, telling AzureLib to pre-scale the model
     */
    public AbstractVisibleTrinketRenderer<T> withScale(float scaleWidth, float scaleHeight) {
    ...
    public void actuallyRender(MatrixStack poseStack, T animatable, BakedGeoModel model, RenderLayer renderType, VertexConsumerProvider bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        ...
        if (!isReRender) {
            ...
            animationState.setData(VisibleTrinketsDataTickets.STRING, this.currentSlotGroup);
            animationState.setData(VisibleTrinketsDataTickets.STRING, this.currentSlotName);
    ...
    protected void applyBoneVisibilityByTrinketSlot(String currentSlotGroup, String currentSlotName) { // TODO trinket slot
        ...
        if (!ItemUtils.isUsable(this.currentStack)) return;
